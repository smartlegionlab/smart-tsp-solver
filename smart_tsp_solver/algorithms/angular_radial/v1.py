...
@njit(fastmath=True, cache=True)
def _run_2opt(route: np.ndarray, dist_matrix: np.ndarray, max_iterations: int = 1000) -> np.ndarray:
    n = len(route)
    ...
    while improved and iteration < max_iterations:
        ...
        for i in range(1, n - 2):
            for j in range(i + 1, n - 1):
                ...
                a, b, c, d = best_route[i - 1], best_route[i], best_route[j], best_route[j + 1]
                current = dist_matrix[a, b] + dist_matrix[c, d]
                proposed = dist_matrix[a, c] + dist_matrix[b, d]

                if proposed < current:
                    best_route = _2opt_swap(best_route, i, j)
...
def angular_radial_tsp_v1(
    ...
    if post_optimize:
        ...
        route = _run_2opt(route, dist_matrix, max_2opt_iter)
